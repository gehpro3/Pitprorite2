...
export const PayoutChart: React.FC<{ onClose: () => void; mode: PayoutChartMode }> = ({ onClose, mode }) => {
    ...
    return (
        ...
        >
            <div
                className="bg-slate-800 rounded-lg shadow-xl max-w-sm w-full max-h-[80vh] overflow-y-auto p-6 relative border-2 border-slate-500"
                onClick={(e) => e.stopPropagation()}
            >
                <h3 className="text-2xl font-bold text-slate-300 mb-4 text-center">{title}</h3>
                <button
                    onClick={onClose}
                    className="absolute top-2 right-2 text-slate-400 hover:text-white text-3xl font-bold"
                    aria-label="Close chart"
                >&times;</button>
                <table className="w-full text-left text-white">
                    <thead>
                        <tr className="border-b border-slate-600">
                            <th className="p-2 text-lg text-slate-300">Bet</th>
                            <th className="p-2 text-lg text-slate-300">Payout</th>
                        </tr>
                    </thead>
                    <tbody>
                        {chartData.map(({ bet, payout }) => (
                            <tr key={bet} className="border-b border-slate-700 hover:bg-slate-700/50">
                                <td className="p-2 font-mono">${bet}</td>
                                <td className="p-2 font-mono text-slate-300">${payout.toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
